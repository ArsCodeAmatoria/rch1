...
const titleEn = "Appendix A — Standard Hand Signals for Crane & Rigging Operations | Rigging Education";
const descriptionEn =
  "Reference appendix: signaling principles, dedicated signaler, emergency stop, hoist/lower/swing/boom/travel/slow/hold, trolley signals, combined movements, blind lifts, radio integration, communication failure, visibility, human factors, and situational awareness for signalers.";
...
const toc = [
  ...
  {id: "emergency-stop-by-any-worker", label: "Anyone may stop"},
  {id: "hoist-signal", label: "HOIST"},
  {id: "lower-signal", label: "LOWER"},
  {id: "swing-left-signal", label: "Swing left"},
  {id: "swing-right-signal", label: "Swing right"},
  {id: "boom-up-signal", label: "Boom up"},
  {id: "boom-down-signal", label: "Boom down"},
  {id: "travel-signal", label: "Travel"},
  {id: "slow-movement-signal", label: "SLOW"},
  {id: "hold-position-signal", label: "Hold position"},
  {id: "trolley-travel-tower-cranes", label: "Trolley (tower)"},
  {id: "trolley-in-signal", label: "Trolley in"},
  {id: "trolley-out-signal", label: "Trolley out"},
  {id: "multiple-function-signals", label: "Multi-function"},
  {id: "blind-lift-signaling-appendix", label: "Blind lifts"},
  {id: "radio-communication-integration", label: "Radio + hands"},
  {id: "communication-failure-procedures", label: "Comm failure"},
  {id: "environmental-factors-signals", label: "Environment"},
  {id: "human-factors-signaling", label: "Human factors"},
  {id: "situational-awareness-signaling", label: "Situational awareness"},
  {id: "final-communication-reminder", label: "Reminder"}
] as const;
...
export async function generateMetadata({params}: {params: Promise<{locale: string}>}): Promise<Metadata> {
  ...
  return buildPageMetadata({
    ...
    keywords: [
        ...
        "tower crane trolley signal",
        "rigger signaling procedures",
        "blind lift signaler"
      ]
    });
}
...
export default async function RiggingEducationAppendixAPage() {
  ...
  const breadcrumb = [
    ...
    {name: "Appendix A — Hand signals", url: pageUrl}
  ];
    ...
    <article className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <JsonLd
        data={buildTechArticleSchema({
          ...
          url: pageUrl,
          locale,
          keywords: ["crane stop signal", "hoist lower hand signal", "dog everything signal", "crane signaler visibility"],
          about: ["Rigging", "Crane signaling", "Communication", "Safety"]
        })}
      />
      ...
      <nav aria-label="Breadcrumb" className="mb-6 text-sm text-muted-foreground">
        ...
        / <span className="text-foreground">Appendix A</span>
      </nav>

      <header className="space-y-3 rounded-xl border border-border bg-card p-6">
        <Badge className="w-fit">Rigging education · Appendix A</Badge>
      ...
      <div className="mt-8 lg:grid lg:grid-cols-[minmax(0,1fr)_280px] lg:gap-12">
        <RiggingEducationAppendixAStandardHandSignals locale={locale} />
