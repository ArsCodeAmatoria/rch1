...
const titleEn = "Module 11 — Multiple Crane & Tandem Lifts | Rigging Education";
const descriptionEn =
  "Educational module: tandem and dual-crane lifts, load sharing and dynamic transfer, COG and capacity, boom deflection, hoisting coordination, communication, lift director, engineered plans, exclusion zones, and situational awareness.";
...
export async function generateMetadata({params}: {params: Promise<{locale: string}>}): Promise<Metadata> {
  ...
  return mergePlaceholderImageMeta(
    ...
    buildPageMetadata({
      locale,
      path: "/rigging/education/module-11",
      title: isFr ? "Module 11 — Levages multi-grues et en tandem" : titleEn,
      description: isFr
        ? "Module pédagogique : partage de charge, coordination et levages critiques multi-grues. (Texte principal en anglais.)"
        : descriptionEn,
      keywords: [
        "tandem crane lift",
        "dual crane lift planning",
        "critical lift multiple cranes",
        "ASME B30 tandem",
        "load sharing two cranes"
      ]
    })
  );
}

export default async function RiggingEducationModule11Page() {
  const locale = (await getLocale()) as "en" | "fr";
  const pageUrl = `${SITE_URL}/${locale}/rigging/education/module-11`;
  ...
  const breadcrumb = [
    ...
    {name: "Module 11 — Multiple crane & tandem lifts", url: pageUrl}
  ];
    ...
    <article className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <JsonLd
        data={buildTechArticleSchema({
          ...
          url: pageUrl,
          locale,
          keywords: ["tandem lift", "multiple crane lift", "lift director", "engineered lift", "ASME B30"],
          about: ["Crane rigging", "Critical lifts", "Construction lifting"]
        })}
      />
      ...
      <nav aria-label="Breadcrumb" className="mb-6 text-sm text-muted-foreground">
        ...
        / <span className="text-foreground">Module 11</span>
      </nav>

      <header className="space-y-3 rounded-xl border border-border bg-card p-6">
        <Badge className="w-fit">Rigging education · Module 11</Badge>
      ...
      <div className="mt-8 lg:grid lg:grid-cols-[minmax(0,1fr)_220px] lg:gap-10">
        <RiggingEducationModule11MultipleCraneTandem locale={locale} />
