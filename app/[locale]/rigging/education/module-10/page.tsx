...
const titleEn = "Module 10 — Chainfalls, Lever Hoists & Tensioning Devices | Rigging Education";
const descriptionEn =
  "Educational module: manual chain hoists (chainfalls), lever hoists and come-alongs, tensioning and stored energy, snap-back, anchoring, hooks and chain inspection, brakes, environment, communication, and temporary stabilization.";
...
export async function generateMetadata({params}: {params: Promise<{locale: string}>}): Promise<Metadata> {
  ...
  return mergePlaceholderImageMeta(
    ...
    buildPageMetadata({
      locale,
      path: "/rigging/education/module-10",
      title: isFr ? "Module 10 — Palans à chaîne, palans à levier et tension" : titleEn,
      description: isFr
        ? "Module pédagogique : palans manuels, palans à levier, énergie emmagasinée et ancrages. (Texte principal en anglais.)"
        : descriptionEn,
      keywords: [
        "chainfall hoist",
...
export default async function RiggingEducationModule10Page() {
  const locale = (await getLocale()) as "en" | "fr";
  const pageUrl = `${SITE_URL}/${locale}/rigging/education/module-10`;
  ...
  const breadcrumb = [
    ...
    {name: "Module 10 — Chainfalls & lever hoists", url: pageUrl}
  ];
    ...
    <article className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <JsonLd
        data={buildTechArticleSchema({
          ...
          url: pageUrl,
          locale,
          keywords: ["manual hoist", "chain hoist inspection", "lever hoist safety", "ASME B30", "stored energy rigging"],
          about: ["Rigging", "Hoisting", "Manual lifting"]
        })}
      />
      ...
      <nav aria-label="Breadcrumb" className="mb-6 text-sm text-muted-foreground">
        ...
        / <span className="text-foreground">Module 10</span>
      </nav>

      <header className="space-y-3 rounded-xl border border-border bg-card p-6">
        <Badge className="w-fit">Rigging education · Module 10</Badge>
      ...
      <div className="mt-8 lg:grid lg:grid-cols-[minmax(0,1fr)_220px] lg:gap-10">
        <RiggingEducationModule10ChainfallsLeverHoists locale={locale} />
