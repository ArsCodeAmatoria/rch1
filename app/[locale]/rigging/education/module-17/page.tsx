...
const titleEn = "Module 17 — Rigging Safety Culture, Human Factors & Operational Decision-Making | Rigging Education";
const descriptionEn =
  "Educational module: safety culture, shared responsibility, human factors, fatigue and complacency, time and production pressure, assumptions and miscommunication, situational awareness, stop work authority, communication and operational discipline, learning from incidents, hazard recognition, decision-making during lifts, leadership, team coordination, environmental stress, continuous improvement, and professionalism.";
...
export async function generateMetadata({params}: {params: Promise<{locale: string}>}): Promise<Metadata> {
  ...
  return buildPageMetadata({
    locale,
    path: "/rigging/education/module-17",
    title: isFr
        ? "Module 17 — Culture de sécurité du montage, facteurs humains et prise de décision opérationnelle"
        : titleEn,
    description: isFr
        ? "Module pédagogique : culture de sécurité, fatigue, pression, autorisation d’arrêt, leadership. (Texte principal en anglais.)"
        : descriptionEn,
    keywords: [
        "rigging safety culture",
        "stop work authority crane",
        "human factors lifting",
        "situational awareness rigging",
        "operational discipline lifting"
      ]
    });
}

export default async function RiggingEducationModule17Page() {
  const locale = (await getLocale()) as "en" | "fr";
  const pageUrl = `${SITE_URL}/${locale}/rigging/education/module-17`;
  ...
  const breadcrumb = [
    ...
    {name: "Module 17 — Safety culture & human factors", url: pageUrl}
  ];
    ...
    <article className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <JsonLd
        data={buildTechArticleSchema({
          ...
          url: pageUrl,
          locale,
          keywords: ["safety culture construction", "near miss reporting", "production pressure safety", "rigging communication"],
          about: ["Rigging", "Safety culture", "Human factors", "Crane lifting"]
        })}
      />
      ...
      <nav aria-label="Breadcrumb" className="mb-6 text-sm text-muted-foreground">
        ...
        / <span className="text-foreground">Module 17</span>
      </nav>

      <header className="space-y-3 rounded-xl border border-border bg-card p-6">
        <Badge className="w-fit">Rigging education · Module 17</Badge>
      ...
      <div className="mt-8 lg:grid lg:grid-cols-[minmax(0,1fr)_280px] lg:gap-12">
        <RiggingEducationModule17SafetyCultureHumanFactors locale={locale} />
